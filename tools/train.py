...
from siamban.utils.visualize import Visual
from siamban.utils.bbox import lt2corner_rect, corner2center
...
def build_opt_lr(model, current_epoch=0):
    ...
    if cfg.MULDECT.MULDECT:
        trainable_params += [{'params': model.multi_det.parameters(),
                              'lr': cfg.TRAIN.BASE_LR}]
...
def train(train_loader, model, optimizer, lr_scheduler, tb_writer):
    ...
    for idx, data in enumerate(train_loader):
        ...
        if cfg.TRAIN.VISUAL:
            outputs, vis_data = model(data, epoch)
        else:
