...
class DepthwiseXCorr(nn.Module):
    ...
    def forward(self, kernel, search, matching_score=None):
        kernel = self.conv_kernel(kernel)
        search = self.conv_search(search)
        feature = xcorr_depthwise(search, kernel)
        if matching_score is not None:
            matching_score = \
                torch.from_numpy(matching_score.reshape(25, 25).astype('float32')).cuda().expand_as(feature)
            feature_ = feature * matching_score
            out = self.head(feature_)
            return out, feature, feature_
        else:
...
class DepthwiseBAN(BAN):
    ...
    def forward(self, z_f, x_f, matching_score=None):
        if matching_score is not None:
            cls, corr_cls_origin, corr_cls = self.cls(z_f, x_f, matching_score)
            loc = self.loc(z_f, x_f)
            return cls, loc, corr_cls_origin, corr_cls
        else:
...
class MultiBAN(BAN):
    ...
    def forward(self, z_fs, x_fs, matching_score=None):
        cls = []
        loc = []
        corr_cls_origin = []
        corr_cls = []
        ...
        for idx, (z_f, x_f) in enumerate(zip(z_fs, x_fs), start=2):
            ...
            if flag:
                c, l, corr_origin, corr = box(z_f, x_f, matching_score)
                corr_cls.append(corr)
                corr_cls_origin.append(corr_origin)
            else:
                c, l = box(z_f, x_f)
            cls.append(c)
        ...
        if flag:
            if self.weighted:  # average cls/reg
                return weighted_avg(cls, cls_weight), weighted_avg(loc, loc_weight), \
                       avg(corr_cls_origin), avg(corr_cls)
            else:
                return avg(cls), avg(loc), avg(corr_cls_origin), avg(corr_cls)
        else:
            if self.weighted:  # average cls/reg
                return weighted_avg(cls, cls_weight), weighted_avg(loc, loc_weight)
            else:
